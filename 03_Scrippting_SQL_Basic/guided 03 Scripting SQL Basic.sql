...
select nmproduk, kategori from produk where idproduk = 'PR001';

-- cari data tabel yang tidak duplicate
select distinct kategori from produk;

--sortir data tabel yang ingin ditampilkan
select platform, count(*) as jumlah_game
from game group by platform;

-- pengurutan data tabel tertentu
select * from produk order by harga asc;

--kalo kita mau tahu tabel apa saja didalam connection atau database yang kita buat
select * from tab;

-- lihat struktur tabel
desc genre;

-- kalusa select, aritmetik
select nmfurnitur, stok + 10 from furnitur where idfurnitur = 'FN005';
select namagame, harga, harga - (harga * 0.02) from game where idgame = 'GM001';
select nmfurnitur, harga, harga - (harga * 0.08) from furnitur where idfurnitur = 'FN007';

-- kolom alias
select namagame as "zyrexgame", platform as device, harga "rupiah" from game;

-- fungsi konkat tabel tertentu
select nmfurnitur || ' - ' || bahan as "info furnitur" from furnitur;

-- fungsi literal tabel tertentu
...
-- operator logika or
select namagame, platform, harga from game where platform = 'PS' or platform = 'Moblie';
...
select nmproduk, kategori from produk where kategori not in ('Elektronik');

-- operator between not between
select namagame, harga from game where harga between 400000 and 800000;
select nmfurnitur, stok from furnitur where stok not between 5 and 10;

-- NULL / not NULL
select namagame, platform, harga from game where harga = 0;
select nmfurnitur, stok from furnitur where stok is not null;

-- operator like / not like
select namagame from game where namagame like 'A%';
select namagame from game where namagame not like '%FIFA%';